...
public class BookActivity extends BaseActivity implements CompoundButton.OnCheckedChangeListener {
    ...
    protected void initData() {
        ...
        for (int i = 0; i < surcharges.size(); i++) {
            ...
            if (surcharges.get(i).getTprice() == null) {
                costBean.setFeename(surcharges.get(i).getName());
                if (surcharges.get(i).getGp20() != null && surcharges.get(i).getGp20().length() > 0) {
                    costBean.getPrices().add(new BoxBean(Config.GP20, Float.parseFloat(surcharges.get(i).getGp20()), 1));
                }
                if (surcharges.get(i).getGp40() != null && surcharges.get(i).getGp40().length() > 0) {
                    costBean.getPrices().add(new BoxBean(Config.GP40, Float.parseFloat(surcharges.get(i).getGp40()), 1));
                }
                if (surcharges.get(i).getHq40() != null && surcharges.get(i).getHq40().length() > 0) {
                    costBean.getPrices().add(new BoxBean(Config.HQ40, Float.parseFloat(surcharges.get(i).getHq40()), 1));
                }
            } else {
                costBean.setType(CostItemBean.Type.TICKET);
                costBean.setFeename(surcharges.get(i).getName());
                costBean.setCost(new BigDecimal(surcharges.get(i).getTprice().trim()));
            }
    ...
    private DatePickerDialog.OnDateSetListener mDateSetListener = new DatePickerDialog.OnDateSetListener() {
        ...
        public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
            ...
            bean.setBgt(date);
            bargeDateView.setText(date + WEEK[calendar.get(java.util.Calendar.DAY_OF_WEEK) - 1]);
        }
    };
